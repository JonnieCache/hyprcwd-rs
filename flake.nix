    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      in
      {
        checks = {
          ...
          # Audit dependencies
          hyprcwd-audit = craneLib.cargoAudit {
            inherit src advisory-db;
          };

          # Audit licenses
          hyprcwd-deny = craneLib.cargoDeny {
            inherit src;
          };

          # Run tests with cargo-nextest
          # Consider setting `doCheck = false` on `hyprcwd` if you do not want
          # the tests to run twice
          hyprcwd-nextest = craneLib.cargoNextest (
            commonArgs
            // {
              inherit cargoArtifacts;
              partitions = 1;
